...
// Filename is capital-K (matches `Kaparthy.md` at the workspace root),
...
/// Base prompt + the `Kaparthy.md` guidelines, joined once at startup.
...
/// it was created — an edit to `Kaparthy.md` or [`SYSTEM_PROMPT_BASE`]
...
mod tests {
    ...
    fn system_prompt_appends_the_guidelines_after_the_base() {
        ...
        assert!(prompt.ends_with(kaparthy::GUIDELINES));
        assert!(!kaparthy::GUIDELINES.trim().is_empty());
    }
